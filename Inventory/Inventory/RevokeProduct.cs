...
namespace Inventory
{
    public partial class RevokeProduct : Form
    {
        public RevokeProduct()
        {
            InitializeComponent();
            FillNames();
        }
        ...
        private void RevokeProductBtn_Click(object sender, EventArgs e)
        {
            int quantity;
            if (int.TryParse(ProductQuantityTxt.Text, out quantity))
            {
                if (Validations.RevokeQuantity(quantity, ProductNameCB.SelectedValue.ToString()))
                {
